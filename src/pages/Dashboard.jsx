  ...
  return (
    <Container className="dashboard-page py-4">
      ...
      <div className="dashboard-header">
        <div>
          <p className="dashboard-overline">Event Management Hub</p>
          <h1>Manage Your Event Bookings</h1>
          <p className="dashboard-intro">
            View your event registrations, track ticket status, and manage payments in one place. Get live insights into all your upcoming events.
          </p>
        </div>
      ...
      <Row className="dashboard-grid dashboard-summary-grid">
        <Col lg={12} sm={6}>
          <DashboardCard icon={<FiTrendingUp />} title="Event Tickets Booked" value={bookings.length} subtitle="Total registrations" />
        </Col>
        <Col lg={12} sm={6}>
          <DashboardCard icon={<FiDollarSign />} title="Total Revenue" value={`₹${analytics.revenue.toLocaleString()}`} subtitle="Ticket sales value" />
        </Col>
        <Col lg={12} sm={6}>
          <DashboardCard icon={<FiCalendar />} title="Upcoming Events" value={analytics.upcoming} subtitle="Events to attend" />
        </Col>
        <Col lg={12} sm={6}>
          <DashboardCard icon={<FiCheckCircle />} title="Cancellations" value={statusCounts.Cancelled} subtitle="Cancelled tickets" accent="accent-danger" />
        </Col>
      </Row>

      <Row className="dashboard-grid dashboard-charts-grid">
        <Col lg={12}>
          <ChartCard title="Event Bookings Over Time" description="Monthly ticket sales trend.">
            {analytics.lineData.length > 0 ? (
              ...
              <div className="empty-chart d-flex align-items-center justify-content-center" style={{height: 320, color: '#94a3b8'}}>No booking tracking data available yet.</div>
            )}
          </ChartCard>
        </Col>

        <Col lg={12}>
          <ChartCard title="Ticket Revenue by Month" description="Monthly revenue from event bookings.">
            {analytics.barData.length > 0 ? (
              ...
              <div className="empty-chart d-flex align-items-center justify-content-center" style={{height: 320, color: '#94a3b8'}}>No revenue data available yet.</div>
            )}
          </ChartCard>
        </Col>

        <Col lg={12}>
          <ChartCard title="Booking Status Overview" description="Current distribution of ticket statuses.">
            {analytics.pieData.reduce((acc, curr) => acc + curr.value, 0) > 0 ? (
              <ResponsiveContainer width="100%" height={320}>
              <PieChart>
              ...
              <div className="empty-chart d-flex align-items-center justify-content-center" style={{height: 320, color: '#94a3b8'}}>No status tracking data available.</div>
            )}
          </ChartCard>
        </Col>
      </Row>
      ...
      <div className="dashboard-toolbar">
        <div className="toolbar-copy">
          ...
          <p>Sort and filter your event bookings with live status and payment options.</p>
        </div>
      ...
      {loading || authLoading ? (
        ...
        <div className="dashboard-empty-state">
          ...
          <p>Discover and book amazing events from our collection.</p>
