  ...
  useMemo(() => {
    let result = events.filter((event) => {
      ...
      if (filters.duration !== "all") {
        const eventDays = parseInt(event.duration);
        const filterDays = parseInt(filters.duration);
        if (filters.duration === "short") matchesDuration = eventDays <= 3;
        if (filters.duration === "medium") matchesDuration = eventDays >= 4 && eventDays <= 7;
        if (filters.duration === "long") matchesDuration = eventDays > 7;
      }

      // Difficulty filter
      const matchesDifficulty =
        filters.difficulty === "all" ||
        event.difficulty.toLowerCase() === filters.difficulty.toLowerCase();

      // Rating filter
      const eventRating = parseFloat(event.rating);
      const matchesRating = eventRating >= filters.rating;
  ...
  return (
    <div className="trip-listing-page">
      ...
      <section className="listing-header">
        <Container>
          <h1 className="listing-title">Discover Amazing Events</h1>
          <p className="listing-subtitle">
            Browse and book from {events.length} carefully curated events
          </p>
        </Container>
      </section>
            ...
            <Col lg={9}>
              ...
              {filteredEvents.length > 0 ? (
                <>
                  <Row className="trips-grid g-4">
                    {paginatedEvents.map((event) => (
                      <Col lg={12} md={6} sm={12} key={event.id}>
                ...
                <div className="no-results">
                  ...
                  <h4 className="no-results-title">No events found</h4>
